...
from .resnet import ResNet_ASTER, extract_g
...
class MODEL(nn.Module):
    ...
    def __init__(self, n_bm, n_vocab, 
        ...
        # print(n_tgt_vocab)      #94
        self.encoder = ResNet_ASTER()
        #self.encoder = resnet34()
        self.extract_g = extract_g(512)
        self.conv1x1 = nn.Conv2d(1024, 1024, kernel_size=1, bias=False)
        self.LR = LR
    ...
    def beam_decode_step(self,inst_dec_beams, len_dec_seq, src_seq, enc_output, global_feat, inst_idx_to_position_map, n_bm,LorR=1):
        ...
        def predict_word(dec_seq, dec_pos, src_seq, enc_output, global_feat, n_active_inst, n_bm, LorR=1):
            if LorR==1:
               dec_output, slf_attns1, enc_attns = self.decoder1(dec_seq, dec_pos, src_seq, enc_output, global_feat, return_attns=True,if_test=True)
               # print('slf_attn1', slf_attns1[0].shape)      #[80,1,1]
               # print('enc_attns', enc_attns[0].shape)  # [80,1,200]
               # heatmap(slf_attns1, enc_attns[0])
            elif LorR==2:
               dec_output, slf_attns2, enc_attns = self.decoder2(dec_seq, dec_pos, src_seq, enc_output, global_feat, return_attns=True,if_test=True)
            else: 
        ...
        word_prob, attn = predict_word(dec_seq, dec_pos, src_seq, enc_output, global_feat, n_active_inst, n_bm, LorR=LorR)
        
        # Update the beam with predicted word prob information and collect incomplete instances
        active_inst_idx_list = collect_active_inst_idx_list(
            inst_dec_beams, word_prob, inst_idx_to_position_map)

        return active_inst_idx_list, attn
    ...
    def forward(self, x, length_ori, text1_ori, text2_ori=None, test=False, cpu_texts=None):
        N = x.shape[0]
        ...
        # Encoder #
        # print('x', x.size())
        cnn_feat, x5 = self.encoder(x)
        #print(cnn_feat.shape)
        #global_feat = self.extract_g(x5)     #(160, 512)
        global_feat = x5
        text1_new = text1_ori
        text2_new = text2_ori
        length_new = length_ori                     
        
        cnn_feat = self.conv1x1(cnn_feat)      #(160, 512, 6 ,20)
        cnn_feat = cnn_feat.squeeze(-1).permute(0, 2, 3, 1).contiguous().view(N, -1, cnn_feat.shape[1])    #(160, 120, 1024)
        ...
        if test==True and self.n_bm >=1:
            ...
            for len_dec_seq in range(1, self.max_seq_len):
                active_inst_idx_list, attn = self.beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, src_enc, global_feat, inst_idx_to_position_map, self.n_bm, LorR=1)
                # print(attn)#.shape)
               # heatmap(attn,attn[])
                if not active_inst_idx_list:# or len_dec_seq==:
                    break
            ...
            # print(batch_scores1)        #tensor([-3.8147e-06], device='cuda:0')]
            ...
            seq_stacked1 = torch.Tensor(seq_stacked).long().cuda()      #bian ma id word id[51, 27, 18, 12, 14, 28, 91,
            ...
            if text2_ori is not None:
               ...
               #-- Decode
               for len_dec_seq in range(1, self.max_seq_len):
                   active_inst_idx_list, attn = self.beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, src_enc, global_feat, inst_idx_to_position_map, self.n_bm, LorR=2)
