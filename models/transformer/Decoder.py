    ...
    def forward(self, tgt_seq, tgt_pos, src_seq, enc_output, global_feat, return_attns=False, if_test=False):
        ...
        for index, dec_layer in enumerate(self.layer_stack):
            dec_output, dec_slf_attn, dec_enc_attn = dec_layer(
                ...
                layer_idx=index)
            # print(dec_enc_attn.shape)
            # heatmap(dec_enc_attn,dec_enc_attn)
            if return_attns:
                # print('dec_enc_attn', dec_enc_attn.shape)       #[80,i,200]

                dec_slf_attn_list += [dec_slf_attn]
                dec_enc_attn_list += [dec_enc_attn]

        heatmap(dec_enc_attn, dec_enc_attn)       #right position

        # for i,xx in enumerate(dec_slf_attn_list):
            # print(xx.shape)
        # sum = torch.zeros(dec_enc_attn.shape)
        # for i in range(len(dec_enc_attn_list)):
        #     sum = torch.add(sum, dec_enc_attn_list[i].cpu())
        # heatmap(sum,sum)
        if return_attns:
